...
local tonumber,course_is_main_course,string_sub,warp_to_warpnode = tonumber,course_is_main_course,string.sub,warp_to_warpnode
...
local function on_warp_command(msg)
    -- convert input into a table of arguments
    ...
    for _, value in pairs(stages) do
        if msg[1] == value[1] or msg[1] == value[2] then
            ...
            act = tonumber(msg[3]) or (course_is_main_course(levelToCourse[level]) and 1 or 0)
            if msg[4] and string_sub(msg[4], 1, 2) == "0x" then
                node = tonumber(string_sub(msg[4], 3, #msg[4]), 16) or 10
            else
